...
export const device = () => {
    ...
    const supportsTouchEvents = onTouchStartInWindow || (touchEventInWindow && matchMedia('(any-pointer: coarse)').matches);
    ...
    const isIPad = matchMedia('(pointer: coarse)').matches &&
    ...
    const hasCoarsePrimaryPointer = (matchMedia('(pointer: coarse)').matches ||
    ...
    const hasAnyHoverOrAnyFinePointer = matchMedia('(any-pointer: fine)').matches ||
    ...
    const inputType = hasTouch && (hasAnyHoverOrAnyFinePointer || !hasCoarsePrimaryPointer)
        ? 'hybrid'
        : hasTouch
            ? 'touchonly'
            : 'mouseonly';
    const primaryInputType = inputType === 'mouseonly'
        ? 'mouse'
        : inputType === 'touchonly'
            ? 'touch'
