...
const timemodification_1 = require("./timemodification");
/**
...
class Note {
    ...
    getStem() {
        return this.element.first('stem')?.content().enum(enums_1.STEMS) ?? null;
    }
    /** Returns the type of note or null when missing or invalid. */
    getType() {
        return this.element.first('type')?.content().enum(enums_1.NOTE_TYPES) ?? null;
    }
    ...
    getLyrics() {
        return this.element.all('lyric').map((element) => new lyric_1.Lyric(element));
    }
    ...
    getNotations() {
        return this.element.all('notations').map((element) => new notations_1.Notations(element));
    }
    ...
    getAccidentalType() {
        return this.element.first('accidental')?.content().enum(enums_1.ACCIDENTAL_TYPES) ?? null;
    }
    ...
    getNotehead() {
        return this.element.first('notehead')?.content().enum(enums_1.NOTEHEADS) ?? null;
    }
    ...
    getBeams() {
        return this.element.all('beam').map((element) => new beam_1.Beam(element));
    }
    /** Returns the time modification of the note. Defaults to null. */
    getTimeModification() {
        const element = this.element.first('time-modification');
        return element ? new timemodification_1.TimeModification(element) : null;
    }
