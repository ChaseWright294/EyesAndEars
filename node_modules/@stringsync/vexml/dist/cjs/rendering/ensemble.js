...
const nooprenderctx_1 = require("./nooprenderctx");
...
class Ensemble {
    ...
    draw(fragmentRender, { x, paddingLeft, paddingRight }) {
        ...
        const ctx = new nooprenderctx_1.NoopRenderContext();
        ...
        for (const partRender of fragmentRender.partRenders) {
            for (const staveRender of partRender.staveRenders) {
                for (const voiceRender of staveRender.voiceRenders) {
                    ...
                    voiceRender.rect = spatial_1.Rect.merge(voiceRender.entryRenders.map((e) => e.rect));
                }
            ...
            partRender.rect = spatial_1.Rect.merge(partRender.staveRenders.map((s) => s.rect));
        }
        fragmentRender.rect = spatial_1.Rect.merge(fragmentRender.partRenders.map((s) => s.rect));
    ...
    overrideVexflowStaveRect(staveRender, x, paddingLeft, paddingRight) {
        ...
        return spatial_1.Rect.merge([new spatial_1.Rect(x, y, w, h), ...voiceRects]);
    }
    ...
    overrideVexflowTabNoteRect(vexflowTabNote) {
        ...
        for (const y of vexflowTabNote.getYs()) {
            const rect = new spatial_1.Rect(x - TAB_RECT_WIDTH / 2, y - TAB_RECT_WIDTH / 2, TAB_RECT_WIDTH, TAB_RECT_WIDTH);
            rects.push(rect);
        }
        return spatial_1.Rect.merge(rects);
    }
    /**
    ...
    hackEntryRenderRect(entryRender, staveRender) {
        ...
        const rect = spatial_1.Rect.fromRectLike(entryRender.vexflowNote.getBoundingBox());
