...
class System {
    ...
    width;
    position;
    constructor(config, log, document, key, width, position) {
        this.config = config;
        this.log = log;
    ...
    render() {
        ...
        const rect = spatial_1.Rect.merge(measureRenders.map((measure) => measure.rect));
        return {
            type: 'system',
            key: this.key,
            rect,
    ...
    renderMeasures(pen) {
        const measureRenders = new Array();
        ...
        for (let measureIndex = 0; measureIndex < measureCount; measureIndex++) {
            const measureKey = { ...this.key, measureIndex };
            const width = measureWidths?.at(measureIndex) || null;
            if (multiRestCount === 0) {
                const measure = new measure_1.Measure(this.config, this.log, this.document, measureKey, pen.position(), width);
    ...
    getMeasureWidths() {
        ...
        const minRequiredMeasureWidths = new Array();
        ...
        for (let measureIndex = 0; measureIndex < measureCount; measureIndex++) {
            ...
            const key = { systemIndex: 0, measureIndex };
            if (multiRestCount === 0) {
                const measureRender = new measure_1.Measure(this.config, this.log, document, key, spatial_1.Point.origin(), null).render();
    ...
    sliceMeasures(document, from, to) {
